...
describe "Graphics" do
  ...
  context "shapes" do
    describe "Point" do
      let(:first) { make_point(10, 20) }
      let(:second) { make_point(10, 20) }
      let(:third) { make_point(20, 10) }
      ...
      it 'implements the :eql and :== methods' do
        expect(first == second).to eq true
        expect(first.eql? second). to eq true
        expect(first.eql? third).to eq false
      end

      it "implements the hash method" do
        expect(first.hash == second.hash).to eq true
        expect(first.hash == third.hash).to eq false
        expect(second.hash == third.hash).to eq false
      end
    end
    ...
    describe "Line" do
      ...
      context "equality" do
        it "implements the :eql and :== methods" do
          first = make_line make_point(1, 5), make_point(25, 2)
          second = make_line make_point(1, 5), make_point(25, 2)
          third = make_line make_point(5, 5), make_point(25, 2)
          expect(first == second).to eq true
          expect(first.eql? second). to eq true
          expect(first.eql? third).to eq false
        end
    ...
    describe "Rectangle" do
      ...
      context "equality" do
        it "implements the :eql and :== methods" do
          first = make_rectangle make_point(1, 5), make_point(25, 2)
          second = make_rectangle make_point(1, 5), make_point(25, 2)
          third = make_rectangle make_point(5, 5), make_point(25, 2)
          expect(first == second).to eq true
          expect(first.eql? second). to eq true
          expect(first.eql? third).to eq false
        end
